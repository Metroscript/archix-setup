...
sudo pacman -Syu --needed --noconfirm rofi-calc
    ...
    sudo pacman -Syu --needed --noconfirm plasma{,-wayland-session} fwupd packagekit-qt5 xdg-desktop-portal-{kde,gtk} gwenview kimageformats qt5-imageformats kamera dolphin{,-plugins} ffmpegthumbs kde{-{inotify-survey,cli-tools},graphics-thumbnailers,network-filesharing} kio-{admin,fuse} purpose kmag kmousetool kontrast colord-kde kcolorchooser kimagemapeditor koko kruler okular skanlite spectacle svgpart kcron khelpcenter kjournald ksystemlog ark filelight kate kbackup kcalc kcharselect kclock kdebugsettings kdf kdialog keditbookmarks keysmith kfind kfloppy kgpg konsole ktimer kweather markdownpart print-manager skanpage sweeper yakuake maliit-keyboard
fi
#Flatpak
