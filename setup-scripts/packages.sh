...
sudo pacman -Syu --needed --noconfirm rofi-calc
    ...
    sudo pacman -Syu --needed --noconfirm plasma-{meta,wayland-session} flatpak-kcm plymouth-kcm fwupd packagekit-qt5 xdg-desktop-portal-{kde,gtk} gwenview kimageformats qt5-imageformats kamera dolphin{,-plugins} ffmpegthumbs kde{-{inotify-survey,cli-tools},graphics-thumbnailers,network-filesharing} kio-{admin,fuse,extras} purpose icoutils libappimage openexr perl taglib kmag kmousetool kontrast colord-kde kcolorchooser koko kruler okular spectacle svgpart kcron khelpcenter ark filelight kate kbackup kcalc kcharselect kclock kdf kdialog keditbookmarks kfind kgpg kweather markdownpart print-manager skanpage sweeper maliit-keyboard
fi
#Flatpak
