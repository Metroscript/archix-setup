...
if [ "$swap" -gt 0 ];then
    if [ "$btrfs" = y ];then
        ...
        printf "/%s/swapfile none swap defaults 0 0\n" "$swapvol" | sudo tee -a /etc/fstab
        cd;else
    ...
    if [ "$res" = y ];then
        ...
        if ! grep 'sleep.conf.d' "$sdir";then
...
if [ "$zram" -gt 0 ];then
    ...
    printf "ACTION==\"add\", KERNEL==\"zram0\", ATTR{comp_algorithm}=\"%s\", ATTR{disksize}=\"%sG\", RUN=\"/usr/bin/mkswap -U clear /dev/zram0\"" "$zramcomp" "$zram" | sudo tee /etc/udev/rules.d/99-zram.rules
    sudo sed -i 's;quiet;zswap.enabled=0 quiet;' /etc/default/grub
    printf "/dev/zram0 none swap defaults,pri=100 0 0\n" | sudo tee -a /etc/fstab
fi
if lsblk | grep -q nvme && ! grep -q nvme_load "$bootdir";then
